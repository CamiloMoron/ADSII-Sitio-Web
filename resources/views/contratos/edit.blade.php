...
@section('content')
<div class="fade-in">
    <h2 class="text-xl font-bold text-white mb-1">Editar Contrato</h2>
    <p class="text-sm mb-6" style="color:#64748b;">Modificar contrato #{{ $contrato->id }}</p>
    ...
    <div class="rounded-xl p-6" style="background:#1e293b;border:1px solid #334155;">
        <form action="{{ route('contratos.update', $contrato) }}" method="POST" class="space-y-4 max-w-2xl">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="clausulas" 
                          ...
                          class="w-full px-3 py-2.5 rounded-lg text-sm text-white outline-none focus:ring-2" 
                          style="background:#0f172a;border:1px solid #334155;--tw-ring-color:#f59e0b;" 
                          placeholder="Ingrese las cláusulas del contrato...">{{ old('clausulas', $contrato->clausulas) }}</textarea>
            </div>
